// Adapted from https://gist.github.com/nfantone/1eaa803772025df69d07f4dbf5df7e58
 ...
 * @const {string[]}
 */
const SHUTDOWN_SIGNALS = ['SIGINT', 'SIGTERM', 'beforeExit'];
 ...
 */
const shutdownListeners: ((signal: string) => Promise<void>)[] = [];
...
const processOnce = (
  signals: string[],
  fn: (() => void) | ((signalOrEvent: string) => Promise<never>)
) => {
 ...
 * @param {string} signalOrEvent The exit signal or event name received on the process.
 */
async function shutdownHandler(signalOrEvent: string): Promise<never> {
...
export function beforeShutdown(
  listener: (signal: string) => Promise<void>
